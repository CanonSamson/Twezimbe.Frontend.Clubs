...
const CasesTable = ({
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["user-cases", fundId || bfId, filters?.filedById],
    queryFn: () => getBfUserCases(fundId || bfId, filters),
    enabled: !!fundId || !!bfId,
  });
  ...
  console.log(data?.data?.data.cases, " data.data?.data.cases");
  const LoadingRow = () => (
              ...
              >
                ...
                <TableCell className="py-2 px-2 hidden tablet-lg:table-cell text-gray-700">
                  {caseItem.beneficiary?.firstName ||
                    caseItem.beneficiary?.beneficiary?.profile?.firstName ||
                    caseItem.filedByUser?.firstName}{" "}
                  {caseItem.beneficiary?.lastName ||
                    caseItem.beneficiary?.beneficiary?.profile?.lastName ||
                    caseItem.filedByUser?.lastName}
                </TableCell>

                <TableCell className="py-2 px-2 hidden tablet-lg:table-cell">
                  <span className={getStatusColor(caseItem.reason || "N/A")}>
                    {caseItem.reason || "N/A"}
                  </span>
                </TableCell>
                ...
                <TableCell className="py-2 px-2 hidden tablet-lg:table-cell text-gray-700">
                  UGX{" "}
                  {formatAmount(
                    caseItem.totalcontributions?.toLocaleString() || "0"
                  )}
                </TableCell>
