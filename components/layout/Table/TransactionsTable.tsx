...
import { getBfWalletTransactions } from "@/api/bereavement-fund/wallet";
...
const StatusBadge = ({ status }: { status: string }) => {
  ...
  // Map the actual status values to our style keys
  const getStatusStyle = () => {
    switch (normalizedStatus) {
      ...
      case "pending":
      case "peding": // Handle the typo in the type definition
...
// Helper function to get amount color based on status and transaction type
const getAmountColor = (transactionType: string, status: string) => {
  const normalizedStatus = status?.toLowerCase();
  
  // DEBIT transactions are always red regardless of status
  if (transactionType === "DEBIT") {
    return "text-red-500";
  }
  
  // For CREDIT transactions, only show green if successful
  if (transactionType === "CREDIT") {
    if (normalizedStatus === "pending" || normalizedStatus === "failed") {
      return ""; // No color for pending/failed credits
    }
    return "text-green-500"; // Green for successful credits
  }
  
  return "";
};

const TransactionsTable = ({ fundId }: { fundId?: string | undefined }) => {
  const { toggleModal, closeAllModals } = useSettingModal();

  const { data, isLoading } = useQuery({
    queryKey: ["transactions-user", fundId],
    queryFn: () => getBfWalletTransactions(fundId as string),
    enabled: !!fundId,
  });
          ...
          <TableBody>
            {!hasTransactions ? (
              ...
              data?.data.transactions.map((transaction) => {
                ...
                const amountColor = getAmountColor(transaction.transactionType, status);
