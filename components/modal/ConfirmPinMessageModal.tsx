...
import { pinMessage } from "@/api/messaging/group/pinMessage";
import { useParams } from "next/navigation";
import { toast } from "sonner";
import { useContextSelector } from "use-context-selector";
import { UserContext, UserContextType } from "@/contexts/user";

const ConfirmPinMessageModal = () => {
  const { modals, modalData, closeModal, toggleModal } = useSettingModal();
  const data = modalData?.confirmPinMessageModal as MessageType | undefined;
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: () => pinMessage(data?.id as string, groupId as string),
    onError: (error) => {
      toast.error(error.message || JSON.stringify(error));
    },
    onSuccess: (data) => {
      closeModal("confirmPinMessageModal");
      toast(data.data.message || "Message has been pinned.", {
  ...
  return (
    <div
      onClick={() => closeModal("confirmPinMessageModal")}
      className={`${
        modals.confirmPinMessageModal ? "flex" : "hidden"
    ...
    >
      ...
      >
        <div className="text-start mb-4">
          <h4 className="text-[20px] font-Inter">Pin Message?</h4>
...
export default ConfirmPinMessageModal;
