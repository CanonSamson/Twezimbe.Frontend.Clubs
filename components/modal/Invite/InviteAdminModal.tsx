...
const InviteAdminModal = () => {
  ...
  const inviteAdminsModal = useMemo(() => {
    return modals.inviteAdminsModal;
  }, [modals.inviteAdminsModal, groupId]);
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["getGroupInviteLink", "ADMIN", groupId],
    queryFn: () =>
      getGroupInviteLink(groupId as string, { permission: "ADMIN" }),
    enabled: !!groupId && !!modals.inviteAdminsModal,
  });
    ...
    <>
      ...
      >
        ...
        <div className="bg-white z-20 duration-500 transition-all relative tablet:w-[440px] font-inter rounded-lg rounded-bl-none overflow-hidden shadow-lg flex flex-col max-tablet:w-full max-tablet:max-w-none max-tablet:rounded-t-lg max-tablet:rounded-b-none max-tablet:mb-0 max-tablet:bottom-0 max-tablet:h-[70vh]">
          ...
          <div className="flex bg-primary text-[12px] tablet:text-[14px] justify-between py-4 px-2 mobile:p-6 tablet:min-w-[300px]">
