...
import { getBfWallet } from '@/api/bereavement-fund/wallet'
...
import { payNegativeTransitionBalance } from '@/api/bereavement-fund/transition'
...
const PayNegativeTransitionModal = () => {
  const { closeModal, modals, modalData } = useSettingModal()
  const { currentUser } = useUser({})
  const bfId = useParams()?.bfId as string
  const groupId = useParams()?.groupId as string

  const data = modalData?.payNegativeTransitionModal
  const totalPendingFee = data?.amount || 0
  const router = useRouter()

  const { mutate, isPending } = useMutation({
    mutationFn: () => {
      return payNegativeTransitionBalance(bfId, data?.id)
    },
    onSuccess: (response: any) => {
      queryClient.invalidateQueries({
        queryKey: ['getTransitionMemberData', bfId]
      })
      console.log('response', response)
      closeModal('payNegativeTransitionModal')
  ...
  const walletBalance = wallet?.data?.data?.balance || 0
  const hasLowBalance = walletBalance <= 0

  const handleFundWallet = () => {
    // Navigate to wallet funding page
    router.push(`/bf/wallet/${groupId}/${bfId}`)
    setTimeout(() => {
      closeModal('payNegativeTransitionModal')
    }, 200)
  }
  ...
  return (
    <div
      className={`fixed inset-0 z-50 flex items-center justify-center ${
        modals.payNegativeTransitionModal ? 'flex' : 'hidden'
      }`}
    >
      <div
        className='fixed inset-0 bg-black opacity-50'
        onClick={() => closeModal('payNegativeTransitionModal')}
      />
      ...
      <div className='bg-white rounded-lg shadow-lg max-h-[70vh] w-full max-w-md overflow-auto z-10'>
        <div className='p-5 mb-8'>
          ...
          <div className='text-center mt-5'>
            ...
            <p className='text-sm font-medium flex text-start justify-start'>
              You have to pay off this fee to keep accessing benefits of the
              fund.
            </p>
          </div>
          ...
          <div className='flex flex-col gap-4'>
              ...
              <div className='flex justify-between items-center'>
                <span
                  className={`text-sm font-medium ${
                    hasLowBalance ? 'text-red-700' : 'text-blue-700'
                  }`}
                >
                  BF Wallet Balance:
                </span>
                <span
                  className={`text-sm font-semibold ${
                    hasLowBalance ? 'text-red-800' : 'text-blue-800'
                  }`}
                >
                  UGX {formatAmount(String(walletBalance))}
                </span>
              </div>

              {/* Fund wallet link for low/zero balance */}
              {hasLowBalance && (
            ...
            {values.amount > 0 && (
              <div className='bg-gray-50 rounded-lg p-4 mt-2'>
                ...
                {walletBalance < chargeDetails.totalAmount && (
          ...
          <div className='flex justify-center mt-5'>
            <AuthButton
              ...
              isLoading={isPending}
              disabled={walletBalance < chargeDetails.totalAmount}
            />
          </div>
        </div>
      </div>
...
export default PayNegativeTransitionModal
