...
import { activateGlobalWallet } from '@/api/global-wallet/index'
...
const Input: React.FC<{
  ...
  const { mutate, isPending } = useMutation({
    mutationKey: ['activateGlobalWallet'],
    mutationFn: activateGlobalWallet,
    onSuccess: () => {
  ...
  return (
    <>
      ...
      <div className='flex bg-white p-3 gap-3 justify-end'>
        ...
        >
          {isPending ? 'Activating wallet...' : 'Activate wallet'}
        </button>
      </div>
    </>
  )
