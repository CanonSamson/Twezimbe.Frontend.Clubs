...
import { fileNewCase } from "@/api/bereavement-fund/file-case";
...
const FileCaseBfForm = () => {
  const { bfId, beneficiaryId } = useParams();
  ...
  const { mutate, isPending } = useMutation({
    ...
    onSuccess: (data) => openModal("submittedModal", data.data),
    onError: (error: any) => {
      console.log(error, "pendingPaymentModal")
      if (error.code == "BENEFICIARY_PENDING_FEES") {
        openModal("pendingPaymentModal", { force: true});
      }
          ...
          {step === 1 && (
            <div className="flex flex-col gap-4">
              <div className="block bg-white tablet-lg:bg-white [@media_(min-width:_769px)]:bg-transparent max-tablet:p-4 rounded-md">
                ...
                <h1 className="font-inter text-[18px] tablet:text-[26px] text-[#000000]">
                  One more thing!
                </h1>
              </div>
          ...
          {step === 3 && (
            <div className="flex flex-col gap-4">
              ...
              <div className="block bg-white tablet-lg:bg-white [@media_(min-width:_769px)]:bg-transparent p-4 rounded-md flex flex-col items-center gap-2 ">
                <CustomAvatar
                  image={coverImage}
                  className="justify-start w-full tablet:w-[400px] tablet-lg:w-[450px] h-[150px] cursor-pointer"
